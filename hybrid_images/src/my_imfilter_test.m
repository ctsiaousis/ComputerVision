...
%% Setup
%test_image = im2single(imread('../data/cameraman.tif'));
test_image = im2single(imread('../data/pair2_joker.png'));
...
identity_image  = im2single(my_imfilter(test_image, identity_filter));
identity_image0  = imfilter(test_image, identity_filter, 'conv');
immse(identity_image, identity_image0)

%% Square filter
K = [1.5 5.7 2.1 1.7 0.5; 0.2 3.7 5.9 6.2 2.3; 0.1 8.2 5.5 4.1 2.3; 0.2 1.4 1.1 3.2 0.1; 0.3 0.2 0.1 10.2 5.7];
K = K / sum(sum(K));

filtered = im2single(my_imfilter(test_image, K));
filtered0 = imfilter(test_image, K, 'conv', 'symmetric');
immse(filtered, filtered0)
...
blur_image = im2single(my_imfilter(test_image, blur_filter));
blur_image0 = imfilter(test_image, blur_filter, 'conv', 'symmetric');
immse(blur_image, blur_image0)
...
sobel_image = im2single(my_imfilter(test_image, sobel_filter));
sobel_image0 = imfilter(test_image, sobel_filter, 'conv', 'symmetric');
immse(sobel_image, sobel_image0)

%% High pass filter (Discrete Laplacian)
laplacian_filter = [0 1 0; 1 -4 1; 0 1 0];

laplacian_image = im2single(my_imfilter(test_image, laplacian_filter));
laplacian_image0 = imfilter(test_image, laplacian_filter, 'conv', 'symmetric');
immse(laplacian_image, laplacian_image0)
